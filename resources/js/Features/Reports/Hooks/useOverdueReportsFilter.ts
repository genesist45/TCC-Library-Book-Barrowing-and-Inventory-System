...
/**
 * Custom hook for managing overdue reports filter state and actions
 * Supports URL persistence for filter state
 */
export function useOverdueReportsFilter() {
    const { url } = usePage();
    ...
    const handleRefresh = useCallback(() => {
        ...
        router.reload({
            only: ['summary', 'overdueBooks', 'membersWithOverdue', 'overdueByCategory'],
            onFinish: () => {
