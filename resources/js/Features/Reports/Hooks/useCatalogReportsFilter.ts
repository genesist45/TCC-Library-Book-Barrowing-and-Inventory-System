...
import type { DateRangeType, DateFilterState } from '../types/catalogReports.d';
...
/**
 * Custom hook for managing catalog reports filter state and actions
 * Supports URL persistence for filter state
 */
export function useCatalogReportsFilter() {
    const { url } = usePage();
    ...
    const handleRefresh = useCallback(() => {
        ...
        router.reload({
            only: ['summary', 'booksByCategory', 'booksByPublisher', 'lowStockBooks', 'availabilityBreakdown'],
            onFinish: () => {
    ...
    const handleApplyFilter = useCallback(() => {
        ...
        router.get(
            ...
            {
                preserveScroll: true,
                // Don't preserve state so new data is fetched
                preserveState: false,
            }
        );
