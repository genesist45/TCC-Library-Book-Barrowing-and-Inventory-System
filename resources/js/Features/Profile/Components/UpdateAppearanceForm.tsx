...
export function UpdateAppearanceForm({ className = '' }: { className?: string }) {
                ...
                <div className="grid grid-cols-3 gap-2 sm:flex sm:flex-wrap">
                    {themes.map(({ id, label, icon: Icon }) => (
                        <button
                            ...
                            className={`inline-flex items-center justify-center gap-1.5 rounded-md border px-3 py-2 text-xs font-semibold uppercase tracking-widest transition-all sm:gap-2 sm:px-4 ${themeMode === id
