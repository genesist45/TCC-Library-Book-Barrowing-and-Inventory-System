...
export const AuthorTable = ({ authors, onView, onEdit, onDelete, isLoading = false }: AuthorTableProps) => {
    return (
        <div className="overflow-x-auto">
            <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200 bg-white transition-colors duration-300 dark:divide-[#3a3a3a] dark:bg-[#2a2a2a]">
                    {isLoading ? (
                        Array.from({ length: authors.length > 0 ? authors.length : 5 }).map((_, index) => (
                        ...
                        authors.map((author) => (
                            <tr key={author.id} className="transition-colors duration-300 hover:bg-gray-50 dark:hover:bg-[#3a3a3a]">
                                ...
                                <td className="hidden whitespace-nowrap px-3 py-2 text-sm sm:table-cell sm:px-4">
                                    <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${author.is_published
